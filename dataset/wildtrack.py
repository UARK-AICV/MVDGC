...
class Wildtrack(Dataset):
    def __init__(self, cfg, transform, istrain):
        ...
        self.worldgrid_T_worldcoord_aug = torch.zeros(4,4) # should be empyty at first
                ...
                for pedestrian in all_pedestrians:
                    ...
                    for cam in range(self.num_cam):
                        if itemgetter('xmin', 'ymin', 'xmax', 'ymax')(pedestrian['views'][cam]) != (-1, -1, -1, -1):
                            ...
                            num_imgs_bbox += 1
                self.world_gt[frame] = (np.array(world_pts)[:,[1,0]], np.array(world_pids)) # Wildtrack needs flip
                self.imgs_gt[frame] = {}
                for cam in range(self.num_cam):
                    # x1y1x2y2
    ...
    def get_intrinsic_extrinsic_matrix(self, camera_i):
        ...
        rvec = np.array(list(map(lambda x: float(x), rvec)), dtype=np.float32)  # [ 1.759099,  0.467101, -0.331699]
        ...
        rotation_matrix, _ = cv2.Rodrigues(rvec)    # [1x3] => [3x3]
    ...
    def get_image_data(self, index, cameras, aug_mat):
        ...
        img_pts_allcams = torch.zeros((len(cameras), self.max_objects, 4), dtype=torch.float32)
        # norm_bev_pts = torch.zeros((self.max_objects, 2), dtype=torch.float32)
