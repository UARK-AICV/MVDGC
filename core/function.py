...
def validate_3d(config, model, loader, output_dir, threshold, num_views=5, device="cpu", epoch=0, frame_id=None):
    ...
    with torch.no_grad():
        ...
        for i, (inputs, meta) in loader_all_frames:
            ...
            # 5 here is for 5 CAMERAS
            inputs = [i.to(device) for i in inputs] # len 5, inputs[0].shape    torch.Size([1, 3, 512, 960])
            meta = [{k: v.to(device) if isinstance(v, torch.Tensor) else v
                            for k, v in t.items()} for t in meta]
        #     meta[0].keys()
        #     dict_keys(['image', 'num_person', 'joints_3d', 'joints_3d_vis', 'joints_3d_voxelpose_pred', 'roots_3d', 
        #    'joints', 'joints_vis', 'center', 'scale', 'rotation', 'camera', 'camera_Intri', 'camera_R', 'camera_focal', 
        #    'camera_T', 'camera_standard_T', 'affine_trans', 'inv_affine_trans', 'aug_trans'])

            # meta[0]['joints_3d'].shape      torch.Size([1, 64, 15, 3])
            # joints_3d_vis                   visible or not
            # joints_3d_voxelpose_pred          pass through layers ???
            # meta[0]['roots_3d']               tensor([[[  -44.0864, -2849.0000,   792.1670],  ???
            # meta[0]['joints'].shape           torch.Size([1, 64, 15, 2])      joints_2d on ??? camera
            # what are center and scale ???
            # _trans relates to augmentation ???
            ...
            arg_temp = np.where(temp[0][:,0,0].detach().cpu().numpy()!=-1)
            
            vis_pred = pred[0][arg_temp]
            indices = nearby_joints_nms(vis_pred,dist_thr,num_nearby_joints_thr)
            print(indices)
            vis_pred = vis_pred[indices]
            ...
            # campus
            ax.set_zlim(zmin=-1000,zmax= 3000)
            ax.set_ylim(ymin=-2000,ymax= 2000)
            ax.set_xlim(xmin=-3000,xmax= 1000)
            
            # for idx in range(pred.shape[1]):
            #     if pred[0][idx][:,4:5].mean()>0.8:
            #         # import ipdb; ipdb.set_trace()
            #         pts3d = pred[0][idx][:,0:3]
            #         # print(pts3d.astype(int))
            #         for pts in pts3d:
            #             ax.scatter3D(pts[0],pts[1],pts[2],marker='o',c='r')
                        
            for idx in range(vis_pred.shape[0]):
                pts3d = vis_pred[idx][:,0:3]
                # print(pts3d.astype(int))
                for pts in pts3d:
                    ax.scatter3D(pts[0],pts[1],pts[2],marker='o',c='r')
